...
export interface AppData {
  ...
  suppressed: string[];
  /**
   * 実績を獲得した日（バッジ ID → `YYYY-MM-DD`）。
   *
   * **ここだけは導出値を保存する。**ほかの数字は記録から何度でも出し直せるが、
   * 「いつ取ったか」は記録の合計からは戻らない。出し直しにすると、
   * 記録を 1 日消したときに過去の獲得まで消えることになり、実績として不自然。
   *
   * 持っていない ID は**この仕組みより前に取ったもの**。日付が無いので
   * 獲得順には並べられず、達成しやすい順（閾値の小さい順）で後ろにまとめる。
   */
...
export interface Preset {
  ...
  exerciseIds: string[];
  /**
   * やる曜日。**空が既定**（曜日を持たないプリセット）。
   *
   * **区別はこれだけ。**「週メニュー」という別のものは無い——曜日を持つものを
   * 日〜土に並べ直して見せている画面があるだけで、月曜のメニューとは
   * 「月曜に使っているプリセット」でしかない。
   *
   * **複数持てる。**「胸の日は月と木」のように週に 2 回やる組み合わせは普通にある。
   * 1 つしか持てないと、同じ中身のプリセットを曜日ぶん複製することになり、
   * 片方だけ直る事故が起きる。
   *
   * 決めた人にだけ効く。**判定はしない**——やらなかった日を数えたり
   * 「今日はやっていません」と出したりした瞬間に、撤回済みの計画型
   * （`docs/design-training.md` §11-3「未達が積むとアプリを開かなくなる」）に戻る。
   * ここが受け持つのは**並べ替えと札**まで。記録画面で今日の曜日のものを先に出す。
   */
  weekdays: Weekday[];
  /**
   * 種目ごとの既定のセット。**空が既定**（持たないプリセット）。
   *
   * **これは「記録に入る値」ではない。**入力欄に**薄く出すだけ**の目安で、
   * 打つまで記録にはならない（`NumericInput` の `fallback` と同じ扱い）。
   * 自動で書き込むと「打っていない数字が記録になる」——`addDayExercises` が
   * 種目だけを入れて値を写さないのと同じ理由（設計 §1.3）。
   *
   * 器は種目が決める。筋トレは `WorkSet`、有酸素は `CardioSet`。
   * `exerciseIds` から外した種目の分は読み込みで落とす（参照先のない値を残さない）。
   */
  defaults: Record<string, SessionSet[]>;
  /**
   * 伏せてあるか。**既定は false。**
   *
   * `Exercise.shelf` の `hidden` と同じ役目で、**選ぶことをやめる印**。
   * 消すことの代わりではない——中身も名前も曜日も既定のセットも残すので、
   * 戻せばそのまま使える。減量期と増量期でメニューを使い分けるような、
   * 「いまは使わないが、また使う」組み合わせのために持つ。
   *
   * 真偽値で足りる。種目が 3 状態（`Shelf`）なのは、マイ種目に入れていない
   * `adhoc` があるためで、プリセットには「作った／作っていない」しかない。
   *
   * 外れるのは**これから組む場面の候補から**だけ（＋ からの呼び出し・
   * 入力欄に薄く出る既定のセット・週メニューの面）。設定の一覧には
   * 「非表示」欄として残り、そこから戻す。
   */
  hidden: boolean;
}

/** 曜日。日曜 = 0（`weekdayIndex` / `startOfWeek` と同じ数え方） */
...
/** マイ種目の棚（`Exercise.shelf`） */
export type Shelf = 'listed' | 'hidden' | 'adhoc';
...
export interface Exercise {
   ...
   * | | 意味 | カタログでの見え方 |
   * | --- | --- | --- |
   * | `listed` | マイ種目。候補に出る | 出さない（追加済み） |
   * | `hidden` | マイ種目だが伏せてある | 「非表示」と添えて出す |
   * | `adhoc` | マイ種目に入れていない | ふつうに出す（まだ追加していない） |
   *
   * **`hidden` と `adhoc` は別の状態。**前は 1 つの真偽値に押し込んでいたが、
   * 「一度入れて、もう使わないから伏せた」と「そもそも入れていない」は違う。
   * 混ぜると、カタログで印を出し分けられず、マイ種目の非表示欄に
   * 入れたことのない種目が並ぶ。
   *
   * `hidden` は **消すことの代わりではなく、選ぶことをやめる印。**
   * 削除は記録ごと消す（元に戻せない）ので、種目を入れ替えるたびに
   * 過去の推移を捨てることになっていた。伏せるだけなら記録はそのまま残る。
   *
   * `adhoc` は **記録が種目の実体を要る**ために持つ状態。1 日だけ試した種目や、
   * プリセットのためだけに足した種目がこれ。部位も数え方も種目側にしか無いので、
   * 実体が無いと定義のない記録になる。
   *
   * **`adhoc` は欠けではない。**マイ種目はお気に入りのようなもので、使い方は
   * 人それぞれ（日々の種目はプリセットに持って、マイ種目は臨時のものだけ、という
   * 置き方もある）。画面では中立に出す——注意の色を使わず、「未登録」とも言わない。
   *
   * 候補から外れるのは `listed` 以外、それも **これから組む場面** だけ
   * （記録画面の種目選び・プリセットの候補・種目の目標を追加）。
   * **過去を読む場面には出したまま**にする（推移・記録一覧・その日のカード・
   * 保存済みプリセットの中身）。記録が残っているのに辿り着けないのは、
   * 論理削除のいちばん悪い形。
   */
  shelf: Shelf;
