...
折れ線には**いまの位置に輪**が付きます。日次のグラフは今日、週次のグラフは今週です。付くのは**線の上だけ**で、日平均の点には付けません。同じ日に印が2つ並ぶと、どちらを読むのか分からなくなるためです。色は系列のまま（体重は青、体脂肪はオレンジ）で、輪だけを重ねます。ここに別の色を足すと、色が実体に固定されている約束が崩れ、読む側が覚えることを増やすためです。いちばん右の点とは限りません（まとめて入力した日や、未来日の記録があるため）。

記録カードの「推移を見る」から、体重と体脂肪率のグラフをその場で開けます。中身は推移画面と同じもので、期間では絞らず過去ぜんぶを出します（種目の推移を記録画面から開くときと同じ作法）。開いている日の点に印が付きます。
...
<p>
  ...
  <img src="docs/screenshot-training-catalog.png" width="380" alt="マイ種目に追加。器具と部位で絞り込み、カタログから選んで手元に置く" />
</p>

筋トレはセット単位で記録します。種目ごとに挙上量、セット数、最大重量、推定1RMの推移を確認できます。

カタログの種目が最初から記録画面に並ぶことはありません。自分が実際にやる種目だけを選んで手元に置きます（件数はカタログの見出しに出ます）。右下の ＋ は押したまま動かすと置き場所を変えられます（読みたい行を塞いだときに逃がせます）。離すと左右どちらかの端に寄り、高さを覚えます。前回のセット構成は種目ごとに複製できますし、過去の記録やプリセットからその日のメニューをまとめて呼び出すこともできます。種目カードには通算の最高記録と、そこまでの残りが出ます。打った値が消える操作（セット行の削除、その日から種目を外す）は確認してから実行します。空の行は失うものがないので確認しません。
...
続けるための仕掛けとして、連続記録日数、記録カレンダー、実績バッジがあります。

カレンダーは**記録画面の先頭**にあります。打つ前に「前はいつ付けたか」を見て、そのまま日付を選べます。押した日に移るので、ヘッダの日付ナビと同じ働きです — あちらが1日ずつ、こちらが面で選びます。**大きな丸は今日**で、**記録した日は数字の下に小さな印**が付きます。2つを同じ形の濃さ違いにすると、「今日はまだ付けていない」が読み取れなくなるので、別の印にしています。体組成は朝夜そろった日を塗り（●）、片方だけの日を中抜き（○）にして区別します。

表示は**2週が既定**で、1週と月も選べます。記録画面の先頭に置くので、開いているぶんだけ入力欄が下がるためです。1週にしないのは、週が暦で区切られているからで、日曜に開くと過去が1日も映りません。2週なら週のどこで開いても直前の記録が入ります。

**セットの入力はダイアログです。**カードは種目名・その日の合計・通算の最高だけを持ち、「編集」から打ちます。カードに入力欄を積んだままだと、種目が増えるほど、いま打つ種目に届くまでスクロールが要るためです。打つあいだは1種目に集中するので、面を分けたほうが入力欄も大きく取れます。

トレーニングの記録画面では、**回復とプリセットを1行の帯にまとめて**います。どちらも種目を打ちはじめる前に見るもので、打っている最中は用が済んでいます。それぞれをカードで積むと、種目カードに届くまでのスクロールがそのぶん伸びます。帯は入口であると同時に要約で、開かなくても「どこが回復しているか」「いまの組み合わせが保存済みか」が読めます。**体組成とトレーニングは切り替えに従って別々に出します** — 以前は体組成の升目にトレーニングを枠で重ねていましたが、1つの面に2つの軸が乗ると、どちらの話をしているのか読むたびに切り替わるためです。

バッジは体組成30種とトレーニング30種で、ホームの切り替えに合わせて出し分けます。記録は端末内にのみ保存され、サーバーへは何も送りません。アカウントは要らず、オフラインで動きます。
...
あわせて、今日この部位をやってよいかを日数で出します。空ける日数はそのセッションのセット数から決めていて、**6セット未満なら翌日**、6〜10セットなら1日空け、11セット以上で2日空けます。最大でも3日です。種目ごとの設定は要りません。

数えるのは**直接やった（主部位の）セット数だけ**で、補助部位は入れません。週の配分は係数込みで数えるので、ここだけ数え方が違います。**疲労と配分は別の話**だからです。配分は「どこをどれだけやったか」なので、ベンチで肩が半セットぶん働いたことを数える意味があります。回復は「明日その部位をやってよいか」で、補助で入ったぶんを直接やった疲労と同じ重みで数えると、実際にはやれる日に空けろと言うことになります（作成者の記録22セッションで突き合わせたところ、空けろと出ていたのに実際はやっていた件数が10件から3件に減りました）。

**少ないセット数でも消耗の大きい種目は、この数え方では見えません。**アブローラーやRDLのような伸張位の負荷が強い種目、レストポーズを使った日、デッドリフトのように結合組織と全身に残る種目です。回復カードには根拠になったセッションを添えてあるので、中身を見て判断してください。翌日の1セット目で同じ重量・回数が出せるかが、いちばん確かな材料です。
...
起動の待ちだけ、`index.html` に表示を持っています。**速く終わったときは何も出しません** — 表示を 400ms 遅らせてあるので、それより前に画面が載れば一度も見えません。一瞬だけ出て消える表示は、待たされるより気になります。記録の読み出し自体は 10 年ぶんでも 50ms 程度で、長いのはバンドルの取得と解析のほうなので、スクリプトの中ではなく HTML に置いています。10 秒を超えたときだけ、次の手（開き直す）を添えます。

画面の切り替えでは読み込みが起きないので、そこには何も出しません。記録はすべてメモリ上にあり、タブの移動は表示を差し替えるだけです。
...
| 部位の空き | 空ける日数を 6セット未満1日、6〜10セット2日、11セット以上3日 で決め、空ける日数から経過日数を引いて出す。**数えるのは主部位のセット数だけ**（補助部位は入れない）。補助部位の係数で端数が出るので、境目は以上／未満で見る。積み上げず1セッションぶんが上限。回復の窓は24〜72時間なので3日を超えない |
...
| 波及行 | セットを打つ面に出す、**その種目の外に及んだぶん**。その日のその種目の記録を**含まない場合と含む場合の差**で出すので、閉じて開き直しても同じものが出る。部位の今週のセット数は常に、部位の空きと種目の目標は跨いだときだけ。通算の最高は行を足さず、最高の行そのものを `60.0 → 62.5` にする。上限3行 |
...
記録は**週（日曜〜土曜）ごとのレコード**に分けて持ちます。値を1つ打ったときに書き直すのが、全期間ではなくその週だけで済みます（10年ぶんで1回の保存が 35.8ms → 0.04ms。記録の量によらず一定です）。分ける目的は書き込みを軽くすることで、読み込みは起動時にすべて読みます — 開始値・最長連続記録・通算といった全期間の数字を出す以上、全部が要るためです。分け方の判断は [docs/design-storage.md](docs/design-storage.md) にまとめています。

**保存するのは生データだけで、計算した値は保存しません。**週平均も推定1RMも部位別セット数も、起動のたびに作り直します。保存すると「種目の設定を変えたのに古い値が残る」経路ができ、画面には数字が出るので気づけません。
...
**ホーム画面への追加を案内します。** 3 日ぶん記録して、まだブラウザのタブで開いている場合だけ出します。オフラインで開けるようになるほか、ブラウザのデータ整理の対象から外れやすくなります。

**書き出していない記録が溜まったら促します。** 前回の書き出しより後の記録が **30 日ぶん**を超えたら、その場で書き出せるカードを出します。見ているのは**まだ書き出していない量**で、前回からの経過時間ではありません。書き出したあと記録していなければ出ませんし、毎日記録していれば 30 日で出ます。**失う量そのものを言える**ようにするためです。
...
値を 1 つ打つたびに走るのは、保存と、記録全体からの導出（日平均・移動平均・週次集計・種目別の集計）です。どちらも**変わった週だけ**を計算し直す形にしてあるので、記録が伸びても増え方は緩やかです。10年ぶん・1,337KB の生成データで、入力 1 回ぶんが 13.2ms（全部を計算し直すと 40.6ms）、保存が 0.04ms（記録の量によらず一定）。1年ぶんなら 1.5ms です。

`npm run bench` で同じ物差しで測り直せます。手を入れるときは、**数字を見てから**決めてください — 実際、途中で重い場所が入れ替わりました（[docs/design-storage.md](docs/design-storage.md) §2.0）。

どちらもホームのいちばん上——体組成／トレーニングの切り替えと現在地のあいだ——に出します。毎日見る数字より下に置くと読まれず、読まれなければ何も守れないためです。
...
src/
  ...
  lib/
    ...
    exerciseCatalog.ts  種目カタログ127種（器具の違いを含めて140通り。選択肢の一覧で、初期データではない）
