...
ヘッダの切り替えで、どの画面も **体組成** と **トレーニング** の2面を持ちます。以下のスクリーンショットは説明用のサンプルデータで、デモに入っている初期データとは別のものです。
...
- **記録は数タップ** — 朝と夜の2枠に打ち込むだけ。過去の日はいつでも遡って直せる
...
<p>
  <img src="docs/screenshot-training-records.png" width="380" alt="トレーニングの記録画面。種目ごとのカードにセットの回数と重量、前回の記録、セット数・推定1RM・挙上量・最高重量" />
...
- **入力の手間を削る** — 前回のセット構成を種目ごとに複製、その日の献立は過去の記録やプリセットからまとめて呼び出せる
- **通算の最高記録が横に出る** — その種目の自己最高と、そこまでの残りをカードの中で示す

### 配分と伸びを見る

<p>
  <img src="docs/screenshot-training-trend.png" width="380" alt="種目別の推移。指標と部位で絞り込み、種目ごとの直近の値と変化量をスパークラインで並べた一覧" />
  <img src="docs/screenshot-training-goals.png" width="380" alt="部位の目標。週のセット数の進捗と、その部位の種目ごとの目標と到達状況" />
</p>

- **部位の配分が見える** — 週 × 部位のセット数と挙上量。どこをサボっているかが分かる
- **種目ごとの目標** — 重量でも回数（秒数）でも決められる。判定は実際に扱えた重量で
- **続けるための仕掛け** — 連続記録日数、28日分の記録カレンダー、実績バッジ（体組成30種・トレーニング30種。ホームの切り替えで出し分け）
- **アカウント不要・オフライン動作** — 記録は端末内にのみ保存。サーバーへ何も送らない

### 構成のおかしさに気づく

種目リストを部位別に眺めているだけでは見えないことがあります。デッドリフト・スクワット・RDL は主部位がバラバラなので、軸荷重が週に何日集まっているかは部位別セット数に出てきません。記録した日付から、そういう配置を知らせます。

- **軸荷重種目が連日になっていないか** — 背骨に荷重を通す種目を続けて置くと出る
- **時間に収まるか** — セット数 × 1セットの時間。回復コストの低い種目にセットが偏ると、時間だけが伸びる

**疲労の量は持ちません。** 見るのは記録した日付と、種目が軸荷重かどうかという真偽値だけです。推定した量を閾値と比べることはしません。設定 > トレーニング > トレーニング種目のレビュー で有効にすると、記録画面に出ます（既定は無効）。

あわせて**部位の空き**——「今日この部位をやってよいか」——を日数で出します。
...
軸荷重種目
  前回  昨日   デッドリフト・スクワット ／ 8/30の週 2日
```

空ける日数はそのセッションのセット数から決めます（5セットまで翌日 / 6〜10セット中1日 / 11セット以上中2日）。**3日を超えません** — 回復の窓は24〜72時間で、それを超えて空けても回復は進まないためです。種目ごとの記入も設定も要らず、部位別セット数（主部位＋補助部位×係数）をそのまま使います。

**軸荷重種目だけは日数に換算しません。** 脊柱起立筋そのものは24〜72時間で戻りますが、椎間板や靭帯は無血管性で回復が数日〜週単位、椎弓・終板に至っては週〜月単位で、そもそも「抜ける」対象ではありません。しかも椎間板の内部はほぼ無神経なので、症状は早期警告として機能しません。だから積み方（いつ置いたか）を見せますが、**それは腰の状態ではありません。**
...
>
> **この初期データが入るのはデモ向けのビルドだけです。**`npm run build` で作ったものには入りません（`npm run build:demo` / `npm run deploy` でだけ `.env.demo` の `VITE_DEMO=1` が渡ります）。自分でホストする場合は空の状態から始まります。
...
| 部位の空き | 空ける日数を `5セットまで1日 / 6〜10セット2日 / 11セット以上3日` で決め、`空ける日数 − 経過日数` を出す。**積み上げない**（1セッションぶんが上限）。**3日を超えない**（回復の窓は24〜72時間） |
| 「何日ぶん」 | `ceil( 残り ÷ 回復量 )`。画面表示用の換算で、判定には使わない |
| セッションの所要時間 | `セット数 × 1セットの時間`。**回数は掛けない**（セットの長さは休憩がほとんどを占めるので、5回でも10回でもあまり変わらない）。±20%ほどずれる粗い見積もり |
...
**構成チェックは検算であって、メニューの提案ではありません。** 組んだ構成に「明らかにおかしい配置」がないかを見るだけで、最適な組み合わせは作りません。判定するのは人の実績ではなく、いま組んでいる構成という設計物です。曜日ごとのメニューも持ちません。いつ・何をやったかという記録そのものを検査の入力にしています。

**疲労の量を数字で持っていません。** 軸荷重も前腕も真偽値で、判定は記録した日付から出します。以前は腰椎に 0〜10 の負荷値を置いて線形減衰させていましたが、1本の数字に時定数の違う4つの層（起立筋24〜72時間 / 神経系48〜96時間 / 椎間板・靭帯 数日〜週 / 骨 週〜月）を代表させていて、どの層としても正しくありませんでした。「あと何日」という言い方は、いちばん戻らないものに「待てば戻る」を含意します。

**アプリが見ているのは積み方であって、体の状態ではありません。** 椎間板の内部はほぼ無神経で、症状は進行してから出ます。だからこそ外形的な記録を見る価値がありますが、数字が静かなことは安全を意味しません。持続する違和感は、この画面ではなく専門医の領域です。
...
src/
  ...
  lib/
    ...
    check.ts            構成チェック（腰部の残存疲労、日内合計、所要時間、部位の疲れ）
    exerciseCatalog.ts  種目カタログ90種（選択肢の一覧。初期データではない）
                        ＋ 構成チェックの値（腰部を使う種目の表と2つのフラグ）
