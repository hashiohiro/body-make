...
記録はブラウザの localStorage にのみ保存され、サーバーへは送信されません。裏を返すと、ブラウザのデータを消せば記録も消えます。設定画面から JSON と CSV で書き出せるので、ときどきバックアップを取ってください。

復元に使えるのは JSON だけです。CSV は元のExcelと同じ列構成、つまり日次記録と週次分析に、筋トレのセット単位の明細と週次の部位別セット数を足したものを BOM 付きで出力します。Excel でそのまま開いてピボットできますが、書き出し専用です。これで復元できると思われると設定が失われるため、読み込みは JSON に限っています。

### 消えないようにしていること

保存先が端末のブラウザだけなので、**記録が消える経路を塞ぐ側にも同じだけ書いています。**

**保存できていないときは画面に出します。** 容量が足りない端末やプライベートモードでは `localStorage` が書けません。以前はここを黙って握っていたので、打った値は画面に出るのに保存されず、次に開いたときにまとめて消えていました。いまは全画面の上部に出して、その場で JSON を書き出せるようにしています。閉じるボタンは置いていません（閉じたあとに打った値も同じように消えるため）。

**ブラウザに「消さないでほしい」と申請します。** 記録が 1 件でも入ると `navigator.storage.persist()` を 1 回だけ呼びます。通るかどうかを決めるのはブラウザで、断られても動作は変わりません。
...
**いま何バイト使っているかを出します。** 設定 > データ の「保存サイズ」です。記録は体組成が 1 日 86 バイト、筋トレが 1 セッション 830 バイトほどで増えます。目安として並べている 5MB は、ブラウザが localStorage に持たせる上限のおおよその値で、**判定には使っていません**（正確な値は端末とブラウザで変わります）。上限と同じ物差しに合わせるため、文字数ではなく UTF-16 の 2 バイト単位で数えています。

天井そのものより先に効くのは書き込みの重さのほうです。いまは値を 1 つ打つたびに記録全体を組み立て直しているので、**1MB を超えたあたりが、日単位のレコードへ割る合図**になります。
