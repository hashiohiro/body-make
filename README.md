...
**体組成とトレーニングを同じグラフに並べない。** 除脂肪体重はインピーダンス式の推定値で振れが大きく、動かす要因もトレーニングだけではありません。同じx軸に並べた時点で「関係がある」と主張することになるため、記録もグラフも別のタブに分けています。
...
src/
  ...
  lib/
    ...
    exerciseCatalog.ts  種目カタログ35種（選択肢の一覧。初期データではない）
